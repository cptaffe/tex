...
{:77}{80:}
  ...
  interrupt := 0;
  OKtointerrupt := true;{:97}{166:}
  {wasmemend:=memmin;waslomax:=memmin;
washimin:=memmax;panicking:=false;}{:166}{215:}
  nestptr := 0;
  maxneststack := 0;
  curlist.modefield := 1;
...
begin
  ...
  overflow(300,memmax+1-memmin);
  40: mem[r].hh.rh := 0;{varused:=varused+s;}
  getnode := r;
  10:
end;{:125}{130:}
...
{if eqtb[5300].int>0 then restoretrace(p,545);}
                ...
                begin{if eqtb[5300].int>0 then restoretrace(p,544);}
                end{:283};
            end;
        end;
...
{666:}
  ...
  50:{663:}if outputactive then print(846)
      else
        begin
          if packbeginline<>0
            then
...
{677:}
  ...
  50:{675:}if outputactive then print(846)
      else
        begin
          if packbeginline<>0
            then
                  ...
                  begin
                    if minimaldemerits[fitclass]<=
                      ...
                      begin
                        ...
                        mem[q].hh.rh := passive;
                        passive := q;
                        mem[q+1].hh.rh := curp;{passnumber:=passnumber+1;
...
if artificialdemerits then printchar(42)else printint(d);end[:856];}
  ...
  10:
{[858:]if curp=printednode then if curp<>0 then if mem[curp].hh.b0=7
...
{954:}
      if triemax<h+256 then
        begin
          if triesize<=h+256 then overflow(950,
                                           triesize);
          repeat
            triemax := triemax+1;
...
{:848};{863:}
  ...
  if threshold>=0 then
    begin{if eqtb[5295].int>0 then begin
...
{876:}
  postlinebreak(finalwidowpenalty){:876};{865:}
  q := mem[29993].hh.rh;
  while q<>29993 do
    begin
      curp := mem[q].hh.rh;
  ...
  80:{1035:}if curl<256 then
              begin
                if mem[curq].hh.rh>0 then if mem[
                                             curlist.tailfield].hh.b1=hyphenchar[mainf]+0 then
                                            insdisc := true;
                if ligaturepresent then
                  begin
...
{1327:}
  ...
  if (x<>69069)or eof(fmtfile)then goto 6666{:1327};
  loadfmtfile := true;
  goto 10;
  6666:;
...
begin
  ...
  if c=1 then
    begin
      for c:=0 to 4 do
        if curmark[c]<>0 then deletetokenref(
                                             curmark[c]);
      if lastglue<>65535 then deleteglueref(lastglue);
      storefmtfile;
      goto 10;
...
{:1058}{1071:}
  ...
  primitive(1064,31,102);{:1071}{1088:}
  primitive(1079,43,1);
  primitive(1080,43,0);{:1088}{1107:}
...
{:1222}{1230:}
  ...
  primitive(940,99,0);
  primitive(952,99,1);{:1250}{1254:}
  primitive(1216,78,0);
  primitive(1217,78,1);{:1254}{1262:}
  primitive(274,100,0);
  primitive(275,100,1);
  primitive(276,100,2);
